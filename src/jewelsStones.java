...
public class jewelsStones
{
    ...
    {
        Map<Double, List<Pair<Integer,Integer>>> slopeLine = new HashMap<>();
           ...
           for(int j=i+1; j<points.length;j++)
             {
                ...
                {
                    if(!slopeLine.get(m).contains(new Pair(points[j][0],points[j][1] )))
                    {
                        List<Pair<Integer, Integer>> line1 = slopeLine.get(m);
                        line1.add(new Pair(points[j][0],points[j][1] ));

                        slopeLine.put(m, line1);
                    }
                    if(!slopeLine.get(m).contains(new Pair(points[i][0],points[i][1] )))
                    {
                        List<Pair<Integer, Integer>> line1 = slopeLine.get(m);
...
int max=0;
        for (Map.Entry<Double, List<Pair<Integer, Integer>>> entry:  slopeLine.entrySet() )
        {
